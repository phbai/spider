...
def insert_posts_with_page(page):
    ...
    for item in reversed(items):
        try:
            ...
            viewkey = re.findall(r'viewkey=(.+?)&', link)[0]
            ...
            author_info = sub_page_soup.find_all(id='videodetails-content')[0].find_all('a')[0]
            author_url = author_info['href']
            author = author_info.find('span').get_text().strip()

            # 检测是否已经存入数据库
            link_result = posts.find_one({"viewkey": viewkey})
            # 数据库中已经存在 直接跳过
            if (link_result):
                continue
